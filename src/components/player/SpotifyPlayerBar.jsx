...
export default function SpotifyPlayerBar() {
    ...
    const handleSpeedChange = (speed) => {
        ...
        const audioEl = window._dreamLabAudio
    ...
    return (
        <>
            ...
            <div
                onClick={() => setShowFullScreen(true)}
                className={`md:hidden fixed bottom-20 left-4 right-4 z-[100] bg-[#181818]/95 backdrop-blur-lg rounded-2xl shadow-2xl transition-transform duration-300 cursor-pointer ${
                    isVisible ? 'translate-y-0' : 'translate-y-full'
                }`}
            >
                <div className="p-3">
                    <div className="flex items-center gap-3 mb-2">
                        <img
                            src={getEpisodeArtwork(currentEpisode, currentBook)}
                            alt={currentEpisode.title}
                            className="w-12 h-12 rounded-md object-cover flex-shrink-0 shadow-md cursor-pointer"
                            onClick={() => setShowFullScreen(true)}
                        />
                        <div className="flex-1 min-w-0">
